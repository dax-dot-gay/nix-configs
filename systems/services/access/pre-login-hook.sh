...
if [ $USER_ID -eq 0 ]; then
    if [ $SFTPGO_LOGIND_PROTOCOL = "OIDC" ]; then
        JQ_OUT=$(/run/current-system/sw/bin/printf '{"status": 1,"username": %s,"has_password": false,"permissions": {"/": ["*"], "/Public": ["list", "download"]},"virtual_folders":[{"id": 10, "name": "Public", "mapped_path": "/shared/data/public", "virtual_path": "/Public"}]}' "${USER_NAME}")
